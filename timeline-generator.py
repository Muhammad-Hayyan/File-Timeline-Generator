...
def scan_disk_image(image_path: str) -> List[TimelineEntry]:
    ...
    if not DISK_IMAGE_SUPPORT:
        print("Error: Disk image parsing requires pytsk3 and pyewf libraries.", file=sys.stderr)
        ...
        for partition in volume:
            if partition.len > 0:
                try:
                    ...
                    timeline_entries.extend(process_directory(fs_info, fs_info.open_dir(path="/")))
                except Exception as e:
                    print(f"Error processing partition {partition.addr}: {e}", file=sys.stderr)
    except Exception as e:
...
def process_directory(fs_info, directory, parent_path="/") -> List[TimelineEntry]:
    ...
    try:
        for entry in directory:
            ...
            if hasattr(entry.info, 'type') and entry.info.type == pytsk3.TSK_FS_NAME_TYPE_DIR:
                sub_directory = fs_info.open_dir(path=file_path)
                timeline_entries.extend(process_directory(fs_info, sub_directory, file_path))
    except Exception as e:
...
def sort_timeline(output_file: str):
    try:
        ...
        sorted_entries = sorted(entries, key=lambda x: x['timestamp'] if x['timestamp'] else '9999-99-99')
...
def main():
    parser = argparse.ArgumentParser(description='Generate a timeline of file operations from a disk image or folder.')
    parser.add_argument('--path', required=True, help='Path to the disk image or folder to analyze')
    parser.add_argument('--output', default='timeline.csv', help='Output CSV file (default: timeline.csv)')
    parser.add_argument('--disk-image', action='store_true', help='Specify if the path is a disk image')
    parser.add_argument('--recursive', action='store_true', help='Recursively scan folder contents')
